...
#   dependents.
RSpec.shared_examples "a dependent index action" do |source_factory, path_method, read_permission|
  subject!(:source) { create(source_factory) }

  let(:entity_dependents_path) { send(path_method, namespace: source.namespace, name: source.name) }
