...
RSpec.describe Plugin do
  ...
  describe "#for_entity" do
    let(:entity_scope) { instance_double(EntityScope) }

    before { allow(EntityScope).to receive(:new).and_return(entity_scope) }

    it "creates an EntityScope for the given context" do
      plugin.for_entity("Domain") { |scope| }

      expect(EntityScope).to have_received(:new).with(plugin, "Domain", nil)
    end
    ...
    it "creates an EntityScope with a condition" do
      ...
      expect(EntityScope).to have_received(:new).with(plugin, "Domain", condition)
    end
  ...
  describe "#actions" do
    let(:entity_scope) { instance_double(EntityScope, actions: [ { path: "/path" } ]) }
  ...
  describe "#annotations_for" do
    let(:entity_scope) { instance_double(EntityScope, annotations: [ { key: "example.com/annotation" } ]) }
  ...
  describe "#components_for" do
    let(:entity_scope) { instance_double(EntityScope, components_for: [ { component: "MyComponent" } ]) }

    before { allow(EntityScope).to receive(:new).and_return(entity_scope) }

    it "returns empty array when context is not registered" do
      expect(plugin.components_for("Domain", :header)).to eq([])
    end
  ...
  describe "#tabs_for" do
    let(:entity_scope) { instance_double(EntityScope, tabs: [ { name: "tab1" } ]) }

    before { allow(EntityScope).to receive(:new).and_return(entity_scope) }

    it "returns empty array when context is not registered" do
      expect(plugin.tabs_for("Project")).to eq([])
    end

    it "returns tabs for registered context" do
      plugin.for_entity("Project") { |scope| }

      expect(plugin.tabs_for("Project")).to eq([ { name: "tab1" } ])
    end
  end
