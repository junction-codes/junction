  ...
  context "when the user is authenticated" do
    ...
    describe "GET /dashboard" do
      ...
      context "with owned entities" do
        before do
          create(:group_membership, user: users(:one), group: group)
      ...
      context "with no recent catalog items" do
        before do
          [ Api, Component, Domain, Resource, System ].each(&:delete_all)
        end
      ...
      context "with group hierarchy" do
        ...
        before do
          create(:group_membership, user: users(:one), group: child_group)
