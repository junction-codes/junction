        ...
        def view_template(&)
          render Table::FilterBar.new(
            ...
            div(class: "grid grid-cols-1 md:grid-cols-4 gap-4") do
              ...
              bar.select_filter(
                name: "q[type_eq]",
                label: Junction::Api.human_attribute_name(:type),
                options: @available_types,
                selected: @query.type_eq,
                include_blank: true,
                blank_label: t(
                  ".all",
                  label: Junction::Api.human_attribute_name(:type).pluralize
                )
              ) if @available_types.any?

              bar.entity_filter(
                name: "q[system_id_eq]",
                label: Junction::Api.human_attribute_name(:system_id),
                entities: @available_systems,
                selected: @query.system_id_eq,
                include_blank: true,
                blank_label: t(
                  ".all",
                  label: Junction::Api.human_attribute_name(:system_id).pluralize
                )
              ) if @available_systems.any?

              bar.entity_filter(
                name: "q[owner_id_eq]",
                label: Junction::Api.human_attribute_name(:owner_id),
                entities: @available_owners,
                selected: @query.owner_id_eq,
                include_blank: true,
                blank_label: t(
                  ".all",
                  label: Junction::Api.human_attribute_name(:owner_id).pluralize
                )
              ) if @available_owners.any?

              bar.select_filter(
                name: "q[lifecycle_eq]",
                label: Junction::Api.human_attribute_name(:lifecycle),
                options: @available_lifecycles,
                selected: @query.lifecycle_eq,
                include_blank: true,
                blank_label: t(
                  ".all",
                  label: Junction::Api.human_attribute_name(:lifecycle).pluralize
                )
