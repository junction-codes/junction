        ...
        def view_template
          ...
          Card do |card|
            ...
            card.content do
              dl(class: "divide-y divide-gray-200 dark:divide-gray-700") do
                metadata_row(@api.class.human_attribute_name(:created_at),
                             @api.created_at.strftime("%b %d, %Y"))
                metadata_row(@api.class.human_attribute_name(:updated_at),
                             @api.updated_at.strftime("%b %d, %Y"))
              end
            end
          end
          ...
          if @can_destroy
            Card(class: "border-red-500/50 dark:border-red-500/30") do |card|
              card.header do
                card.title(class: "text-red-700 dark:text-red-400") { t(".danger_zone_title") }
              end
                ...
                Dialog do |dialog|
                  ...
                  dialog.content do |content|
                    ...
                    content.footer do
                      ...
                      Link(variant: :destructive, href: junction_catalog_path(@api), data_turbo_method: :delete) { t(".confirm_delete") }
                    end
                  end
                end
              end
