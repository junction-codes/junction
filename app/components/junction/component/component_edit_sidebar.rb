        ...
        render Components::Card.new do |card|
          ...
          card.content do
            dl(class: "divide-y divide-gray-200 dark:divide-gray-700") do
              metadata_row("Created At", @component.created_at.strftime("%b %d, %Y"))
              metadata_row("Last Updated", @component.updated_at.strftime("%b %d, %Y"))
            end
          end
        end
            ...
            render Dialog do |dialog|
              dialog.trigger do
                render Button.new(variant: :destructive, class: "w-full justify-center") do
                  ...
                  plain "Delete Component"
                end
              end

              dialog.content do |content|
                content.header { |h| h.title { "Are you absolutely sure?" } }
                content.body { "This will permanently remove the component." }
                content.footer do
                  render Link.new(data: { action: "click->ruby-ui--dialog#dismiss" }) { "Cancel" }
                  render Link.new(variant: :destructive, href: component_path(@component), data_turbo_method: :delete) { "Confirm Delete" }
                end
              end
            end
          end
