          ...
          Card do |card|
            ...
            card.content do
              dl(class: "divide-y divide-gray-200 dark:divide-gray-700") do
                metadata_row(@domain.class.human_attribute_name(:created_at),
                             @domain.created_at.strftime("%b %d, %Y"))
                metadata_row(@domain.class.human_attribute_name(:updated_at),
                             @domain.updated_at.strftime("%b %d, %Y"))
              end
            end
          end
                ...
                Dialog do |dialog|
                  ...
                  dialog.content do |content|
                    ...
                    content.footer do
                      ...
                      Link(variant: :destructive, href: junction_catalog_path(@domain), data_turbo_method: :delete) { t(".confirm_delete") }
                    end
                  end
                end
              end
