...
module Junction
  module Components
    class DomainEditSidebar < Base
      def initialize(domain:, can_destroy: true)
        @domain = domain
        ...
        render Components::Card.new do |card|
          ...
          card.content do
            dl(class: "divide-y divide-gray-200 dark:divide-gray-700") do
              metadata_row("Created At", @domain.created_at.strftime("%b %d, %Y"))
              metadata_row("Last Updated", @domain.updated_at.strftime("%b %d, %Y"))
            end
          end
        end
            ...
            card.content(class: "space-y-4") do
              ...
              render Dialog do |dialog|
                ...
                dialog.content do |content|
                  ...
                  content.body do
                    "This action cannot be undone. This will permanently remove the domain and all of its associated data."
                  end

                  content.footer do
                    render Link.new(data: { action: "click->ruby-ui--dialog#dismiss" }) { "Cancel" }
                    render Link.new(variant: :destructive, href: domain_path(@domain), data_turbo_method: :delete) { "Confirm Delete" }
                  end
                end
              end
            end
