...
class Views::Components::Show < Views::Base
  ...
  def component_header
    div(class: "flex justify-between items-start") do
      ...
      div(class: "flex items-center space-x-6") do
        ...
        div do
          if @component.domain
            Link(href: domain_path(@component.domain), class: "text-sm text-blue-600 hover:underline dark:text-blue-400") do
              "Part of the '#{@component.domain.name}' Domain"
            end
          end
        end
      end
  ...
  def dependencies_table
    table(class: "min-w-full divide-y divide-gray-200 dark:divide-gray-700") do
      ...
      tbody(class: "bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700") do
        @component.dependencies.each do |component|
          tr(class: "hover:bg-gray-50 dark:hover:bg-gray-700/50") do
  ...
  def dependents_table
    table(class: "min-w-full divide-y divide-gray-200 dark:divide-gray-700") do
      ...
      tbody(class: "bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700") do
        @component.dependents.each do |component|
          tr(class: "hover:bg-gray-50 dark:hover:bg-gray-700/50") do
            td(class: "px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900 dark:text-white") { component.name }
            td(class: "px-6 py-4 whitespace-nowrap") do
              render Components::Badge.new(variant: component.status.to_sym) { component.status.capitalize }
