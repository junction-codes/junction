  ...
  module Views
    module Domains
      ...
      class Systems < Views::Base
        ...
        def initialize(systems:, pagy:, query:, page_url:, per_page_url:,
                       sort_url:)
          @systems = systems
          @pagy = pagy
          @query = query
          ...
          turbo_frame_tag "domain_systems" do
            Table(class: "rounded-lg shadow overflow-hidden") do |table|
              table.header do |header|
                header.row do |row|
                  row.sortable_head(query:, field: "name", sort_url:) { t("views.domains.systems.name") }
                  row.sortable_head(query:, field: "status", sort_url:) { t("views.domains.systems.status") }
                end
              end
